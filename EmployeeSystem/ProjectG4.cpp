//Deem Alsuwailem, Dana Alotaibi, Joud Alahmari, Reema Alkorbi, Zainab Mohamed, Reef Alwarthan
/*
...
using namespace std;
//Global variables
const int SIZE=50; //global constant
...
void display_emp();
void add_data();
void save_emp(Employees emps[], int count);
...
{   total[0]++; //statistical report counter
	...
	while(checknum(emps[count].Salary))
    {
       cout<<"Please enter a DIGIT !";
       cout<<"\n Their salary : ";
       cin>>emps[count].Salary;
...
void report_emps()
{
	...
	time_t t;	
	t= time(NULL);
	char *time=ctime(&t); //calling by reference 
